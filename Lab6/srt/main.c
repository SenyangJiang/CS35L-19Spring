...
void* parallel_func(void* index){
    ...
    for( int px=thread_index; px<width; px+=nthreads )
    {
        ...
        for( int py=0; py<height; ++py )
        {
             ...
             */
            all_scaled_color[px][py][0] = scaled_color[0];
            all_scaled_color[px][py][1] = scaled_color[1];
            all_scaled_color[px][py][2] = scaled_color[2];
        }
    }
    return NULL;
}
...
main( int argc, char **argv )
{
    ...
    int* thread_index = (int*)malloc(sizeof(int)*nthreads);

    for(int t = 0; t < nthreads; t++){
        thread_index[t] = t;
        if(pthread_create(&tids[t], NULL, parallel_func, &thread_index[t]))
        {
    ...
    for(int t = 0; t < nthreads; t++){
      if(pthread_join(tids[t], NULL))
        {
            fprintf(stderr, "ERROR: fail to join threads");
            exit(1);
        }
    }
    ...
    for(int i = 0; i < width; i++){
        for(int j = 0; j < height; j++){
            printf( "%.0f %.0f %.0f\n",
                all_scaled_color[i][j][0], 
                all_scaled_color[i][j][1], 
                all_scaled_color[i][j][2] );
        }
        printf( "\n" );
    }
